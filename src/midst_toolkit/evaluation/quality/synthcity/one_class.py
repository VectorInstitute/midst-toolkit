...
class OneClassLayer(BaseNet):
    ...
    def fit(self, x_train: torch.Tensor) -> None:
        ...
        for epoch in range(self.epochs):
            ...
            r = self.r.clone().detach().to(self.device) if isinstance(self.r, torch.Tensor) else torch.tensor([self.r], device=self.device)
            nu = self.nu.clone().detach().to(self.device) if isinstance(self.nu, torch.Tensor) else torch.tensor([self.nu], device=self.device)
