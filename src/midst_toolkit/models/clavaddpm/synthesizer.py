...
def sample_from_diffusion(
    ...
    if dataset.categorical_features is None:
        ...
        # The diffusion model holds the category sizes it was trained with, which is what the input
        # dimension has to agree with when the categorical columns are modelled as categoricals.
        category_sizes = list(diffusion.num_classes)
...
def _post_process_synthetic_data(
    ...
    if num_features == 0 and dataset.categorical_features is None:
        numerical_features = np.array([])
        categorical_features = np.array([])
    else:
        numerical_features, categorical_features = _decode_synthetic_features(
            synthetic_features,
            num_features,
            dataset,
            label_encoders,
...
def _decode_synthetic_features(
    synthetic_features: np.ndarray,
    ...
    """
    Decode the features generated by the diffusion model back into the original data space.

    Which columns of ``synthetic_features`` need to be inverse transformed depends on how the dataset was
    built. If the categorical features were merged into the numerical ones, every column was normalized
    together and the whole row has to be inverse transformed before the trailing columns are decoded back
    into categories. If they were kept as categorical features, only the leading numerical columns were
    normalized and the trailing columns are already category ids.

    Args:
        synthetic_features: The features generated by the diffusion model.
        num_numerical_features: The number of numerical features in the dataset.
        dataset: The dataset object containing the numerical transformations.
        label_encoders: The label encoders used to encode the categorical features.
        is_target_conditioned: The condition on the y column.

    Returns:
        A tuple containing the decoded numerical features and the decoded categorical features.
    """
    if dataset.categorical_features is None:
        ...
        return numerical_features, _decode_categorical_features(encoded_categorical_features, label_encoders)

    numerical_features = synthetic_features[:, :num_numerical_features]
    if dataset.numerical_transform is not None:
        numerical_features = dataset.numerical_transform.inverse_transform(numerical_features)

    encoded_categorical_features = synthetic_features[:, num_numerical_features:]
    return numerical_features, _decode_categorical_features(encoded_categorical_features, label_encoders)
