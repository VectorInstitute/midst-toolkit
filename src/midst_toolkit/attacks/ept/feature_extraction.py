...
def main(
    ...
    1. For each attribute (column) in the synthetic data, train an attribute prediction model using the synthetic data.
    2. Use the trained model to predict the values of that attribute in the challenge data.
    ...
    Args:
        synthetic_data: Synthetic data generated by the target model. This is the data we want to extract features from.
        challenge_data: The data the predictions are compared against. Using this data, we can compute prediction accuracy/errors.
        column_types: A dictionary specifying the types of columns (numerical or categorical) in the data.
        random_seed: Random seed for reproducability. Defaults to None.
    ...
    Returns:
        ...
        If the data is categorical:
            - <column_name>_accuracy: The accuracy of the predictions.
        If the data is numerical:
            - <column_name>_error (if regression): The absolute errors of the predictions.
            - <column_name>_error_ratio (if regression): The ratio of the errors to the true values.
    """
    features = []
    columns = []
