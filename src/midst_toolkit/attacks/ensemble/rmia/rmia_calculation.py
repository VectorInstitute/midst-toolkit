...
def compute_gower_batched(
    ...
    Note: `gower.gower_matrix` performs normalization internally on numeric features.
    Computed distances are divided by the range of all numeric values to normalize them between 0 and 1.
    Therefore, make sure all the max and min values of numeric columns as well as the set of all categories in
    categorical columns are represented in each batch. Since `df_y` exists in all the batches, all you need to
    ensure is that the minimum and maximum values of the numerical columns as well as all the categories exist
    in `df_y`.

    Note: there seems to be a bug in `gower.gower_matrix` when `df_x` and `df_y` have the same number of rows.
    Therefore, make sure `len(data_x) != len(data_y)` when calling `gower.gower_matrix`. This can be achieved by
    changing the batch size to a value not equal to `len(data_y)`.

    More details on the bug:
    `gower.gower_matrix` assumes when `data_x` and `data_y` have the same number of rows, they are the same dataframe,
    and basically computes the distance between `data_y` to itself in an efficient way. But, that is not the intended
    ...
    gower_matrix = np.zeros((n_x, n_y), dtype=np.float32)
    ...
    for start_idx in range(0, n_x, batch_size):
        ...
        gower_matrix[start_idx:end_idx] = batch_matrix.astype(np.float32)
...
def compute_gower_for_model(
    ...
    Args:
        ...
        batch_processing: Whether to use batched computation to reduce peak memory usage.
    ...
    if batch_processing:
        gower_matrix = compute_gower_batched(df_input, df_synthetic, categorical_features, batch_size=5000)
    else:
...
def get_rmia_gower(
    ...
    args_list = [
        ...
        for df_synthetic in model_data
    ]

    # Process in parallel or sequentially based on use_multiprocessing
    if use_multiprocessing:
        with Pool(processes=n_jobs) as pool:
            results = pool.starmap(compute_gower_for_model, args_list)
    else:
        results = [compute_gower_for_model(*args) for args in args_list]
...
def calculate_rmia_signals(
    ...
    shadow_training_id_data = [set(id_list) for id_list in shadow_training_data_ids]
