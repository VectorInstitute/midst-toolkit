...
def compute_gower_batched(
    ...
    then stacks results. Much more memory-efficient for large df_x.
    """
    n_x = len(df_x)
    n_y = len(df_y)
...
def compute_gower_for_model(args):
    ...
    #     data_x=df_input, data_y=df_synthetic, cat_features=categorical_features
    # ).astype(dtype)
...
def calculate_rmia_signals(
    ...
    del shadow_synthetic_list_1

    shadow_model_gower_0_and_1 = get_rmia_gower(
        df_input=df_input,
        model_data=shadow_0_and_1_synthetic_data,
        min_length=min_length,
        categorical_column_names=categorical_column_names,
        id_column_name=id_column_name,
        random_seed=random_seed,
    ...
    # log(INFO, "Computed Gower distance for second set of fine-tuned shadow models.")
    ...
    gower_shadows = np.vstack(
        [np.array(shadow_model_gower_0_and_1), np.array(shadow_model_gower_2)]
    )
    del shadow_model_gower_0_and_1
    del shadow_model_gower_2
    ...
    del fine_tuned_shadow_data_0
    del fine_tuned_shadow_data_1
    del trained_shadow_data
    ...
    del k_nearest_shadow_distances
