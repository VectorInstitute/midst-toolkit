"""
Cross-dataset MIA attack v3:
- Train MLP on berka folds 1-30
- Test MLP on diabetes folds 1-10
"""
import os
import numpy as np
import torch
...
from midst_toolkit.attacks.tartan_federer.tartan_federer_attack import (
    get_score, MLP, DEVICE, CustomUnpickler
)
...
bk_probe = CustomUnpickler(open(BK_BASE / "tabddpm_1" / MODEL_SUBDIR / "None_trans_ckpt.pkl", "rb")).load()
bk_noise_dim = bk_probe.diffusion.num_numerical_features
print(f"Berka noise dim: {bk_noise_dim}")

di_probe = CustomUnpickler(open(DI_BASE / "tabddpm_1" / MODEL_SUBDIR / "None_trans_ckpt.pkl", "rb")).load()
di_noise_dim = di_probe.diffusion.num_numerical_features
...
# Step 1: Collect berka training features (folds 1-30)
print("\n=== Collecting berka training features (folds 1-30) ===")
bk_input_noise = [np.random.normal(size=bk_noise_dim).tolist() for _ in range(NUM_NOISE)]

x_train_list = []
y_train_list = []

for fold_i in range(1, 21):
...
# Step 2: Train MLP on berka data
print("\n=== Training MLP on berka data ===")
...
# Evaluate on berka training data
model.eval()
with torch.no_grad():
...
print(f"\nBerka train AUC: {train_auc:.4f}, max TPR @ 10% FPR: {max_tpr_train:.4f}")

# Step 3: Evaluate on diabetes test folds (1-10)
print("\n=== Evaluating on diabetes test folds (1-10) ===")
...
print(f"\nBerka train AUC: {train_auc:.4f}, max TPR @ 10% FPR: {max_tpr_train:.4f}")
print(f"Diabetes test AUC: {test_auc:.4f}, max TPR @ 10% FPR: {max_tpr_test:.4f}")

os.makedirs("whitebox_cross_DI_berka/cross_attack_results_v3", exist_ok=True)
with open("whitebox_cross_DI_berka/cross_attack_results_v3/mia_performance.txt", "w") as f:
    f.write(f"Cross-dataset MIA v3 (Train: Berka 1-30, Test: Diabetes 1-10)\n")
    f.write(f"Berka train AUC: {train_auc:.4f}, max TPR @ 10% FPR: {max_tpr_train:.4f}\n")
    f.write(f"Diabetes test AUC: {test_auc:.4f}, max TPR @ 10% FPR: {max_tpr_test:.4f}\n")

print("\nDone! Results saved.")

# Score distribution analysis
print("\n=== Attack Score Distribution (Diabetes test) ===")
...
members_di = test_preds[y_test == 1]
nonmembers_di = test_preds[y_test == 0]
print(f"  Members mean:     {members_di.mean():.6f}")
print(f"  Non-members mean: {nonmembers_di.mean():.6f}")

print("\n=== Attack Score Distribution (Berka train) ===")
...
members_bk = train_preds[y_train == 1]
nonmembers_bk = train_preds[y_train == 0]
