"""
Cross-dataset MIA attack v1 with score analysis:
- Train MLP on diabetes folds 1-10
- Test MLP on berka folds 21-30
"""
import os
import numpy as np
import torch
...
# Step 1: Collect diabetes training features (folds 1-10)
print("\n=== Collecting diabetes training features (folds 1-10) ===")
...
# Step 2: Train MLP on diabetes data
...
# Step 3: Evaluate on berka folds 21-30
print("\n=== Evaluating on berka test folds (21-30) ===")
bk_input_noise = [np.random.normal(size=bk_noise_dim).tolist() for _ in range(NUM_NOISE)]

x_test_list = []
y_test_list = []

for fold_i in range(21, 31):
...
# Score distribution
print("\n=== Attack Score Distribution (Berka test folds 21-30) ===")
...
members = test_preds[y_test == 1]
nonmembers = test_preds[y_test == 0]
print(f"  Members mean:     {members.mean():.6f}")
print(f"  Non-members mean: {nonmembers.mean():.6f}")
...
os.makedirs("whitebox_cross_DI_berka/cross_attack_results_v1", exist_ok=True)
with open("whitebox_cross_DI_berka/cross_attack_results_v1/mia_performance.txt", "w") as f:
    f.write(f"Cross-dataset MIA v1 (Train: Diabetes 1-10, Test: Berka 21-30)\n")
