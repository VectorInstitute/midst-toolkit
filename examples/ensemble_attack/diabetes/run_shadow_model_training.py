...
def run_shadow_model_training(config: DictConfig, df_challenge_train: pd.DataFrame) -> list[Path]:
    ...
    assert "encounter_id" in df_challenge_train.columns, (
        "encounter_id column should be present in master train data for the shadow model pipeline."
    )
    assert "encounter_id" in df_population_with_challenge.columns
